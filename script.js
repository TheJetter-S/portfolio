...
document.addEventListener('DOMContentLoaded', () => {
    ...
    updateHeaderScroll(); // Run on load
    ...
    function updateNavPill(activeLink) {
        ...
        // Only work on desktop (pill is hidden on mobile)
    ...
    const initialActive = document.querySelector('.nav-links .nav-link.active');
    if (initialActive) {
        // Delay to ensure layout is ready
        setTimeout(() => updateNavPill(initialActive), 300);
    }

    // Update pill on click
    ...
    window.addEventListener('resize', () => {
        const current = document.querySelector('.nav-links .nav-link.active');
        if (current) updateNavPill(current);
    });
...
(function initializeEnhancedAnimations() {
    ...
    function init() {
        ...
        updateHeaderState(); // Check on load
